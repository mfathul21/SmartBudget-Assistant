...
with app.app_context():
    try:
        ...
        if 'ocr_enabled' not in columns:
            ...
            db.session.execute(text('ALTER TABLE users ADD COLUMN ocr_enabled BOOLEAN DEFAULT false'))
            db.session.commit()
        ...
        if 'image_urls' not in columns:
            ...
            db.session.execute(text('ALTER TABLE users ADD COLUMN image_urls TEXT DEFAULT NULL'))
            db.session.commit()
